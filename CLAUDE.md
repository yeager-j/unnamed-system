...
│   ├── character-sheet/       The v2 entity sheet (S2a — UNN-557, Showtime! redesign): sheet.tsx mounts EntityWriteProvider over the loaded triple; rail/ (identity + archetype switcher, vitals, victories, controls incl. rest-dialog, prisma, exhaustion), mechanics/ (per-mechanic widgets over descriptor dispatch), combat/ (affinity strip + ResolvedSkill banner cards + element-tokens), tab-dock + the ⌘K command-palette. S2b–d add the Explore/Inventory/Archetypes tabs.
