...
└── lib/
    ...
    ├── character/             The v2 character read side (ADR §2.6; UNN-556): load.ts is the one load boundary (`loadCharacterByShortId` → { profile, entity, resolved }) the builder (and the S2 sheet) mount; view/ builders land with S2. See character/CLAUDE.md.
