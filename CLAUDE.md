...
│   ├── combat/                **Shared combat UI kit** (UNN-492): the route-agnostic combat components rendered by BOTH the mapless encounter (components/encounter/) and the spatial dungeon combat canvas (components/dungeon/) — it depends on neither experience. Holds the combatant-drawer (+ actions/conditions/counters/engagement/position/vitals sections), combatant-rail (+ row), watch-sheet-column (+ combat-state-display, conditions-controls), the dialogs (start-combat / end-combat / end-of-turn-modal), turn-order-strip, import-pcs-panel (+ setup-panels), engagement-control (+ side-toggle), zone-enchantment-control, enemy-statblock, and the whole enemies/ bestiary (UNN-346): a three-column master-detail (searchable/family-filtered/level-grouped list + statblock card + a localStorage-backed "Queued enemies" staging rail via hooks/use-encounter-enemy-queue), whose EnemyCatalogPanel is the shared inner surface (the mapless browser wrapper lives in encounter/, the dungeon dialog in dungeon/). The browse statblock card and the DM drawer's enemy section share one renderer — `enemy-statblock.tsx` (`EnemyStatblock`), fed by the engine's resolved `Statblock` (UNN-350 dedup). Plus the **headless** pieces: use-combat-console, dispatch-combat-event, pc-ping, watch-sheet-refresh. **combat conditions (ailments + battle conditions) are DM-only** — conditions-controls is the shared presentational editor; the DM drawer's combatant-conditions-section is its sole editing caller (UNN-467), the watch renders it read-only.
