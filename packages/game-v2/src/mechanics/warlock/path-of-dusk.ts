 ...
 * (entering Dusk Mode); Dark Skills consume Lumina for Dark damage and SP refund
 * (rulebook `Path of Dusk.md`). State holds only the Dusk Mode flag — the player
 * toggles it. Per-enemy Lumina is tracked at the table, so the mechanic is
 * display-only (no engine effects).
 */
