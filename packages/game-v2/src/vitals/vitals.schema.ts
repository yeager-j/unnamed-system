 ...
 * stays honest for %-of-max / threshold rules. Each *operation*
 * (`vitals/operations.ts`) owns its own clamp. Defaults to `0` (full HP) so a
 * pre-PR3 `{ base }` blob still loads (D3, additive migration).
 *
 * Storage is unbounded in game terms but not in machine ones: `z.number().int()`
 * accepts exactly the **safe** integers, so `[MIN_SAFE_INTEGER, MAX_SAFE_INTEGER]`
 * is the real domain, and the accumulating ops saturate there rather than emit a
 * value this schema would reject on the next read.
 */
