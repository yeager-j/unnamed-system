...
import {
  ...
  DAMAGE_TYPES,
} from "@workspace/game-v2/kernel/vocab/affinity"
import { DELIVERIES } from "@workspace/game-v2/kernel/vocab/attack"
import { LINEAGES } from "@workspace/game-v2/kernel/vocab/lineage"
import { SKILL_KINDS } from "@workspace/game-v2/kernel/vocab/skills"

/**
 * The composable **effects** primitive (D32) — re-homed into the v2 kernel as the
 * shared substrate that items and composed Skills both build on. Static,
 * always-on effect primitives: a domain composes its own effect union from these
 * without importing across the others.
 *
 * Effects optionally carry a `source` label used by the UI to break down where a
 * bonus comes from (e.g. "Perfection (B)" contributing +2 to the Attack Roll).
 * Items and passive Skills omit it; mechanic emitters supply it.
 */
...
/**
 * Optional gate restricting when an Attack Roll effect applies. Each axis is a
 * positive list: an axis matches when it is omitted, or when the contextual
 * value is one of the listed values. Multiple axes intersect (all must match).
 * An empty filter matches every Attack Roll.
 */
 ...
 * `perPartyLineage` scaler exists (Magic Circle, Ailment Boost); the kind field
 * reserves a discriminator for future scaler shapes.
 *
 ...
 *
 * `when` filters when the bonus applies (per-Skill / per-weapon); `amount` and
 * `scaler` are mutually exclusive — exactly one must be present.
 */
 ...
 * this is **display-only**: the resolver folds matching effects into the labelled
 * bonus lines the Skill card renders. There is no resolved-damage total today.
 */
 ...
 * the resolve fold). Items, passive Skills, and mechanics keep composing their
 * own unions; this is the neutral name for "an effect from anywhere".
 */
