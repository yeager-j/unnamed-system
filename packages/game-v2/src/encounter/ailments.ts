...
/**
 * A canonical Ailment with its display fields — engine-owned reference content homed
 * in the game package (CH10). Ailments already carry engine semantics next door (the
 * Burn/Sleep end-of-turn ticks in {@link import("./end-of-turn").ailmentHpDelta}, keyed
 * off {@link AILMENT_KEYS}), so this is the display prose joining its mechanics rather
 * than a separate app-side map. `description` is player-facing rules text; the effect
 * mechanics live encoded in that prose (the rulebook's own wording), copied verbatim
 * from v1 `data/combat/ailments.ts`.
 */
...
/** Every canonical Ailment, in {@link AILMENT_KEYS} order. */
export const AILMENTS: readonly Ailment[] = Object.values(AILMENTS_BY_KEY)

/**
 * Looks up a canonical Ailment by its slug key. Returns `undefined` when no Ailment
 * matches.
 */
