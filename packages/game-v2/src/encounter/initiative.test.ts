...
describe("compareInitiative (R3 / CD9a — uniform resolve, no kind branch)", () => {
  it("takes each side's highest Agility and highest Luck independently", () => {
    ...
    const comparison = compareInitiative(participants, resolve)
  ...
  it("a non-empty side beats an empty opposing side", () => {
    ...
    expect(compareInitiative(participants, resolve).suggested).toBe("enemies")
  })
  ...
  it("negative Agility still beats an empty opposing side", () => {
    ...
    expect(compareInitiative(participants, resolve).suggested).toBe("players")
  })
  ...
  it("returns null for a true tie through Luck (the DM-d20 case)", () => {
    ...
    expect(compareInitiative(participants, resolve).suggested).toBeNull()
  })

  it("returns null when both sides are empty", () => {
    const { participants, resolve } = makeScene([])
    expect(compareInitiative(participants, resolve)).toEqual({
