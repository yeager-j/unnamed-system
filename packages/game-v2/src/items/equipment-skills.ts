 ...
 *
 * Both polarities fall out of collecting the granted Skill here: an **active** grant
 * becomes castable (it hydrates into the skill list), a **passive** grant additionally
 * folds its own `effects[]` into the resolve pool — the collection partitions by
 * `isPassive`, so neither is special-cased here. The item-local `skill` effect is
 * never itself a `CombatantEffect`; its only job is to name the granted Skill.
 *
