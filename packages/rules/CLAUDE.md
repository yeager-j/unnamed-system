...
| `1. Players/3. Combat/3.6 Follow-Ups, Shifting, & Showtime!.md` | Follow-Ups on Down (Standard Action only, no Move or Reaction); edge cases (Down + Downed simultaneously on same attack = no Follow-Up); Shifts; Showtime! (combined Almighty damage to all enemies; Ailment/Fallen characters excluded); Prime Time: fully decoupled from Downing enemies and Showtime! — usable when a single chain of Shifts reaches every party member during the current turn; all participants must be **healthy (no Ailment, not Fallen)**; combined damage; only enemies that took damage lose Downed status |
...
- **Showtime!**: when all enemies Downed; each ally rolls their weapon damage + higher of Str/Mag; combined total applied as Almighty damage to every enemy; ailment/Fallen characters excluded; no Follow-Up after
- **Prime Time**: fully decoupled from Downing enemies and from Showtime! — triggers when a single chain of Shifts reaches every party member during the current turn; the character who receives the Follow-Up at the end of that chain may spend it on the party's Prime Time instead of a normal Standard Action; all participants must be **healthy (no Ailment, not Fallen)**; all participants pay SP and roll damage; combined total; only enemies that took damage lose Downed status
