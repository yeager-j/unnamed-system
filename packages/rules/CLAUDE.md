...
| `1. Players/3. Combat/3.6 Follow-Ups, Shifting, & Showtime!.md` | Follow-Ups on Down (Standard Action only, no Move or Reaction); edge cases (Down + Downed simultaneously on same attack = no Follow-Up); Shifts; Showtime! (whole-party effort: combined Almighty damage to all enemies; Ailment/Fallen characters excluded); Prime Time (solo ultimate, fully decoupled from Downing enemies and Showtime!): usable when a single chain of Shifts reaches every party member during the current turn, by the character who receives the Follow-Up at the end of that chain; rest of party fuels it (SP, combined damage) but it's that one character's Skill; all participants must be **healthy (no Ailment, not Fallen)**; only enemies that took damage lose Downed status |
| `1. Players/3. Combat/3.7 Ailments, Technicals, & Saving Throws.md`  | One ailment at a time (most recent takes priority); **duration via saving throw**: roll 1d20+Lu at end of each turn, succeed (>10) = cured; clear after combat; Downed exception: **Downed can coexist with another ailment simultaneously**, clears at start of very next turn; character with ailment cannot receive a Shift or participate in a Showtime!; individual ailment files transcluded; Technicals table; **Saving Throws** section (flavor text todo) |
...
Actual Archetype stat blocks (Attribute bonuses, Affinities, Skill lists, Prime Time).
...
- **Prime Time** Skills listed on Archetype pages; cannot be inherited
...
- **Showtime!**: a whole-party effort, when all enemies Downed; each ally rolls their weapon damage + higher of Str/Mag; combined total applied as Almighty damage to every enemy; ailment/Fallen characters excluded; no Follow-Up after
- **Prime Time**: a solo ultimate, fully decoupled from Downing enemies and from Showtime! — triggers when a single chain of Shifts reaches every party member during the current turn; the character who receives the Follow-Up at the end of that chain may call their own Prime Time (a Skill unique to their Archetype) instead of taking a normal Standard Action; the rest of the party fuels it (pays SP, contributes to combined damage) but it's that one character's moment; all participants must be **healthy (no Ailment, not Fallen)**; only enemies that took damage lose Downed status

**Ailments**
- One at a time (most recent takes priority); end via **saving throw** (1d20 + Lu > 10 at end of each turn); clear after combat
- A character with an Ailment cannot receive a Shift or participate in a Showtime!
...
**Spoils**
- After a combat that ends with a Showtime! or Prime Time, one player rolls 1d4+1 and draws that many cards from the Spoils Deck (recommended: the player who initiated the finishing move)
...
- **"All-Out Attack"** renamed to **"Showtime!"** throughout (the game system itself is now named after this mechanic); the Bard's former "Showtime!" Skill was renamed to **"Grand Finale"** to resolve the name collision; file 3.6 renamed to `3.6 Follow-Ups, Shifting, & Showtime!.md`
- **"Synthesis Skills"** renamed to **"Prime Time"** throughout, with a rule change: Prime Time is now fully decoupled from Downing enemies and from Showtime! — it triggers when a single unbroken chain of Shifts reaches every member of the party during the current turn, and the character who receives the Follow-Up at the end of that chain may call their Prime Time instead of taking a normal Standard Action. This rewards Shifting a Follow-Up around instead of hoarding it. The `synthesis: true` frontmatter key on the relevant Skill files was renamed to `primeTime: true`
- **Framing**: Showtime! is a whole-party effort (every ally rolls damage together); Prime Time is a solo ultimate — a Skill unique to one character's Archetype, called by whoever ends the Shift chain, with the rest of the party fueling it (SP cost, combined damage) rather than performing it together
