...
import {
  ...
  type ThiefsInsightState,
} from "@workspace/game/foundation/mechanics/schema"

/**
 * Thief — Thief's Insight. By studying targets you learn their Tells; the
 * number you can learn about a single enemy equals your Thief Rank. Each Tell
 * grants +1 to Attack Rolls against that enemy, and some Skills gain extra
 * effects past a Tell threshold (rulebook `Skills/Mechanics/Thief's Insight.md`).
 *
 * Tells are per-enemy and the Suspicion check is a transient d12 roll, so —
 * like Path of Dawn's per-enemy Lumina — that data is tracked at the table,
 * not in the app. There is no per-character state to persist, so the mechanic
 * is display-only: it owns its info card and Combat-tab widget but emits no
 * Effects and exposes no write path.
 */
