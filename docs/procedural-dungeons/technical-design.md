...
- **The Region owns two folds**, both written by `finishExpeditionAction` (new — today's finish is a bare `setDungeonStatusAction` flip; status → done + folds, one `guardMany` over dungeon + region, instance read-only; refuses under a live encounter):
...
| `region/create` | region (insert) | No instance minted — expeditions own theirs (D5) |
...
| `dungeon/finish-expedition` (new) | dungeon + region (`guardMany`) | Not an extension — today's finish is a bare `setDungeonStatusAction` flip. Status → done + both folds (`discoveredSiteKeys`, `staticReveal`); refuses under a live encounter |
...
- **R2 — `/stage` shell** (D9): route group, side-nav, `/maps` → `/stage/maps` redirects, empty Sets slot. Sequenced **before** R1's editor-UI half — both touch the `app/maps` route-tree files (`map-editor.tsx`, page components); the shared canvas kit in `components/shared/canvas` is unaffected, so R1's schema/domain half is parallel-safe.
