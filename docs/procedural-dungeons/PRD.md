...
**Status:** Final · **Owner:** Jackson · Full technical design (schemas, the generation algorithm's realization, the pages substrate, the persistent-place model) lives in **[technical-design.md](./technical-design.md)** (2026-07-08 design session; its D5 persistent-place decision supersedes this PRD's reveal-seeding language, as noted inline below). · **Scope boundary:** all cross-expedition *campaign* state — faction clocks, NPC agendas, downtime, the passage of days — belongs to the **Campaign Planner** ([../campaign-planner/PRD.md](../campaign-planner/PRD.md)). This feature owns what happens *inside* one expedition plus one narrow cross-expedition object (the **Region**, below); the Planner owns what happens between expeditions. The two must never grow overlapping clocks.
...
- **Spatial persistence between expeditions.** No landmarks, no saved layout. The reshuffle is the fiction. (Knowledge persists via the Region's discovered-sites list and re-declarable objectives; static-dungeon interiors persist because they live on the Region's persistent instance — tech design D5.)
...
> **Superseded by technical design D5 (2026-07-08):** the Region additionally owns **the persistent place** — one long-lived MapInstance holding the seed pages, grafted static pages, and their reveal state for the campaign's life. Expeditions *visit* it (their dungeon rows reference the shared instance); expedition end **sweeps by provenance** (generated space dies; authored space persists with its fog). This replaced the fold-based reveal-seeding described below and shrank the Region's cross-expedition memory to `discoveredSiteKeys` alone.
...
**Depth.** The **entrance** is wherever the party is placed at delve start (depth 0). Authored zones take depth = graph distance from the entrance, computed at delve start; a minted zone's depth = its parent's + 1, assigned at mint.
...
- **Trivially provable**: within K qualifying expansions, full stop. Testable with a counter.
...
**Sequencing is social, not software.** If the fiction gates one site behind another ("find the key, then the vault"), the DM simply declares them in order — the second when the first resolves, or next expedition. No dependency modeling (see Non-Goals). Multiple *independent* declarations may run concurrently; if two are force-eligible on the same expansion, one places and the other takes the next. The honest conditional, stated plainly: the guarantee is *"within K expansions past min depth"* — a party that never pushes past the min never finds it. That's correct fiction (you must delve), and the console's objective status keeps the DM aware.
...
The Region also remembers **discovery**: sites the party has revealed at least once (appended automatically on first reveal) are annotated on the checklist — "the party knows of: Castle Entrance, the Reliquary, …" — which is informational, not a gate; known-establishment sites are declarable without ever having been found. Mid-run, any site can be **force-placed**: the objective mechanism with **K=1** — *"the next expansion (or the next at depth ≥ D) is this site"* — or the DM picks a specific existing stub directly. Spatially coherent by construction (the site always arrives attached to the party's actual frontier), and "you spend the morning navigating by rumor" has a real price: the expansions along the way each cost their turn. Nothing about the *map* persists — only the memory of what exists.
...
**Portals** compose pages with generation. A portal zone (template-rolled, force-placed, or **drawn directly into any authored Map** — a per-zone `portalMapId` binding in the editor, useful even for fully hand-authored campaigns) references a static Map. The DM's *Enter* action **grafts** that Map's geometry into the running instance as new page(s), stitches the cross-page connection, and places the party at the static entrance. Grafting is **idempotent per static Map, ever** (per technical design D5 — the pages live on the Region's persistent instance): leaving and re-entering, this expedition or three expeditions later, finds the same pages; a second portal to the same Map stitches a new cross-page connection, never duplicates. Consequences, all by construction rather than by rule:
...
- **Static interiors persist in the way that matters:** the *layout* is the authored Map (never reshuffles), and — per technical design D5 — grafted pages live on the Region's **persistent instance**, so the party's mapping of the stable building simply never resets. (This PRD originally specified copying reveal state between per-expedition instances; the persistent-place model made the copy unnecessary — the state was never destroyed.)
...
4. Every generation act (expand, force-pick, force-place, retract, objective placement, contents roll, wandering roll, combat staging) resolves in **one DM gesture** on the hot path — no modal ceremony and no dead clicks (empty-pool fallback included) mid-session.
...
- **Clock semantics across portals: one continuous clock**, by construction (pages, single dungeon row); grafts are idempotent per static Map, ever (tech design D5).
- **Expedition reset cadence: generated space resets; authored space persists** (tech design D5, 2026-07-08): the Region owns a persistent instance, expedition end sweeps by provenance, and static-dungeon fog survives because it's never destroyed — superseding this PRD's earlier "everything resets + re-seed" model.
...
- **Objective guarantee: pre-committed draw** (secret index N ∈ 1..K over qualifying expansions, where qualifying = minted-zone depth ≥ min), replacing a probability ramp. Declared templates leave the random pool; placement overrides adjacency; the **mint ledger** unifies uniqueness and declaration resolution across all placement paths. **Force-place = the same draw with K=1.**
