...
- Players editing their own state. The DM drives everything; the player view is read-only.
...
On victory, the tracker restores Fallen PCs to 1 HP (a vitals write) and clears the encounter's combat state — ailments and battle conditions live on the session, so ending the encounter discards them. The DM closes out the encounter, with a hook for whatever post-combat flow (e.g. Spoils) the app later adds.
...
- **Session reducer is the sole writer of combat state:** Expiry and all combat-state transitions mutate the combatant directly; the only tracker→character writes are PC vitals (HP/SP).
