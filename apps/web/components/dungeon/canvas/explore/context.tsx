...
import type { DungeonConsoleMode } from "@/components/dungeon/canvas/mode-toggle"

/**
 * The run-console state + dispatchers the canvas-internal chrome reads — the
 * {@link import("@/components/dungeon/canvas/explore/zone-node").DungeonZoneNode} toolbar and the
 * {@link import("@/components/dungeon/canvas/explore/turn-loop-bar").TurnLoopBar} (a React Flow `Panel`, so it can
 * own the zoom controls). **Provided by the run console**
 * ({@link import("@/components/dungeon/run-console").DungeonRunConsole}), above
 * {@link import("@/components/dungeon/canvas/canvas").DungeonCanvas} — a context rather than props so
