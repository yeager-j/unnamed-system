...
import {
  getWarriorInheritanceSlots,
  inheritanceSlotsTarget,
  inheritanceTargetArchetypeId,
  resetInheritanceSlotsTarget,
} from "./fixtures/inheritance-slots-target"
...
const CHARACTER_URL = `${inheritanceSlotsTarget.url}?tab=archetypes`
...
test.describe("owner Inheritance Slot configuration", () => {
  ...
  test.beforeEach(async () => {
    await resetInheritanceSlotsTarget()
  })
  ...
  test("configuring a slot persists it and threads the Skill into Combat", async ({
    ...
    await expect.poll(getWarriorInheritanceSlots).toEqual([
      {
        slotIndex: 0,
        sourceCharacterArchetypeId: inheritanceTargetArchetypeId("mage"),
        skillKey: "agi",
      },
    ])
  ...
  test("clearing a slot drops the inherited Skill", async ({ page }) => {
    ...
    await expect
      .poll(getWarriorInheritanceSlots)
      .toEqual([
  ...
  test("read-only viewer sees slot contents but no edit control", async ({
    ...
    try {
      await reader.goto(CHARACTER_URL)
