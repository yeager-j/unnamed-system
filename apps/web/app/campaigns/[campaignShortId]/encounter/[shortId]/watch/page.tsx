 ...
 *  page (its row read is itself React-cached inside the query). The
 *  `campaignShortId` pairing check 404s a watch URL whose campaign doesn't own
 *  this encounter. */
const getSnapshot = cache(
  async (
    campaignShortId: string,
    shortId: string
  ): Promise<EncounterSnapshotResult | null> => {
    const result = await getEncounterSnapshot(shortId, campaignShortId)
...
export async function generateMetadata({
  ...
  const result = await getSnapshot(campaignShortId, shortId)
...
/**
 * The signed-out-visible **player watch view** at `/campaigns/{c}/encounter/{e}/watch`
 * (ADR Decision 5; UNN-322 → UNN-535 on v2). The server loads the **redacted**
