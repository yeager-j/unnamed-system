...
└── writes/          Per-concern persistence wrappers for the **non-character** aggregates — campaign, dungeon, encounter, map, template-set, map-instance — over the shared `guarded-update` (`guardedVersionUpdate`, the single-`version` optimistic-concurrency guard) plus the `guard-many` cross-row transaction helper
```
...
Map and Template Set wrappers accept the Headcanon authority attempt's executor.
Their commands load the current row, reduce intent, guard the version observed in
that same attempt, and stamp the accepted revision; client callers never supply
the token.
