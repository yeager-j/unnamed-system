 ...
 * and its reaction is refreshed. It does **not** set `hasActedThisRound` — that
 * is `endTurn`'s job once the turn is over. A no-op when the combatant id is
 * unknown (Immer returns the original session). The engine never blocks an
 * "ineligible" pick (ADR Decision 8); eligibility is the UI's advisory highlight
 * via `nextDraftingSide`.
 *
 * The reaction refresh is the start-of-turn hook (UNN-308), tied to
 * `draftCombatant` precisely because that fires only for a **normal** turn — a
 * Follow-Up is a bonus Standard Action that grants no Reaction ("regaining use of
 * it at the start of your turn — not a Follow-Up", rulebook 3.1). v1 routes
