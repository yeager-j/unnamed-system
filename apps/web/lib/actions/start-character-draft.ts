 ...
 * and returns its public `shortId` so the client can push to
 * `/builder/{shortId}/basic-info`. Each call creates a new draft —
 * the builder is intentionally multi-draft (a player exploring two character
 * concepts shouldn't have to throw one away to try the other; the existing
 * delete dialog handles cleanup).
 *
