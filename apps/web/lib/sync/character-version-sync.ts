 ...
 * extract the touched version map, which the `EntityWriteProvider` then forwards
 * into its per-class refs via {@link forwardPingedVersions} (UNN-569). The
 * refresh decision + echo suppression live in that forward-only compare (UNN-374).
 */
...
/**
 * `EntityWriteProvider`'s forward-only ping ingest (UNN-569). Advances each
 * class ref to the pinged version iff strictly fresher; keys
