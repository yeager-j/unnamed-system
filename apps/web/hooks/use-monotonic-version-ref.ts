 ...
 * back below a token a write or a ping already advanced it to. The multi-class
 * {@link import("./version-token-store").VersionTokenStore} encodes the same
 * forward-only invariant for the sheet/console; this hook is the single-lane
 * version the builder draft, the rest dialog, and `useQueuedWrite` compose.
 *
