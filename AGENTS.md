...
- **Display labels live in `apps/web/domain/labels.ts`.** Any `Record<X, string>` map that turns a domain key into a human-readable string (damage types, attributes, lineages, ranges, etc.) goes there — don't redefine inline, even for a one-off consumer.
...
- When you need to flip the signed-in/signed-out state in the built-in browser during UI work, open `/` and click **Dev sign in** in the signed-out home panel; use the normal account-menu **Sign out** action to reset. The dev button is server-rendered only for a configured local development host. Automation clients that can issue POST requests may still use `/api/dev/sign-in` and `/api/dev/sign-out` directly — recipe in the route JSDocs. Don't try to delete the session cookie from JS (it's httpOnly).
...
│   ├── shared/                Cross-feature primitives: DetailSection, ResolvedSkillRow + its popover subsystem, Prose, the shared sheet-widget set the sheet + combat watch render (SectionLabel, VitalsBlock, AdjustPoolControl, AffinityStrip, SkillCastSection, mechanics/ — promoted here in UNN-609), shared/canvas/ (route-agnostic React Flow primitives the Map editor + dungeon console share, UNN-464 — now also home to the whole MapCanvas composite, promoted from app/maps in UNN-611; set-piece/ is the one tiered zone card all five zone-node wrappers render (ZoneSetPiece + motif-icons + occupant-chips), tier.ts is the pure semantic-zoom derivation stamped as data-tier — Dungeon Visual Overhaul P1b), and shared/sheet-cards/ (SheetCard + the Identity/Talents/Virtues explore cards + RankUpDialog, shared by the character sheet + the dungeon delve explore column, UNN-611).
│   ├── editor/                Markdown editor primitives shared by sheet + builder.
│   ├── animus/                The narrative writer kit (UNN-221): the sidebar + full-width Markdown pane (WriterSidebar/WriterPane + AnimusDocumentProvider + the entity-door AnimusDocumentEditor + the column-door NotesDocumentEditor) shared by the builder's Movement 3 and the sheet's `/animus` route. Its pure ref model lives in domain/character/animus/documents.ts.
...
│   ├── dungeon/              App-side shaping for the dungeon **exploration** console (UNN-597): console-optimistic.ts — the explore console's single optimistic container over `{ dungeon, instance }` (the dungeon *combat* phase reuses combat/console-optimistic). view/set-piece-view.ts holds the per-surface builders that turn a zone + occupancy into a ZoneSetPieceView (owned from ownedCharacterIds, engagementGroup multi-member only — Dungeon Visual Overhaul P1b). The home the procedural-dungeons feature grows into (docs/procedural-dungeons/ — region/expedition view builders + loaders; the pure generation engine stays in packages/game-v2/src/generation/).
│   ├── map/                  Pure renderer vocabulary for the canvas set-piece (Dungeon Visual Overhaul, docs/dungeon-visual-overhaul/): view/footprints.ts turns a Zone's authored `size` into a fixed world-rect (ZONE_FOOTPRINTS/footprintOf) + the derivations riding on it (zoneTokenCapacity, overlappingZonePairs); view/set-piece-view.ts is the card's view vocabulary (ZoneSetPieceView/SetPieceOccupant, ZoneMotif/ZoneMood aliasing the engine enums — P1b). ZoneSize/motif/mood alias the engine enums; the engine-free canvas kit imports these downward. The live-instance builders live in domain/dungeon/view/.
│   ├── character/            The v2 character read side (ADR §2.6; UNN-556/557): load.ts is the one load boundary (`loadCharacterByShortId` → { profile, entity, resolved }) the builder + sheet mount; view/ holds the pure per-surface builders (rail-view, affinity-strip, archetypes-tab, inventory-table, virtues-card). builder-steps.ts + builder-step-gates.ts (builder domain logic read by lib + domain) and identity-trait-messages.ts moved here in UNN-610. animus/documents.ts (UNN-221) is the writer's pure ref model (DocumentRef + buildDocumentGroups/resolveDocumentContent + the `?doc=` parse/serialize), shared by the components/animus kit. See character/CLAUDE.md.
│   ├── planner/              Campaign Planner pure selectors (tech-design §4, UNN-574): clock-time selectors (horizonOf, isFrozenDay), seasonOf's inherit-forward scan, the slot-materialization planner the clock writes share, DEFAULT_SLOT_TEMPLATE. DB-free, unit-tested.
│   ├── labels.ts             Canonical display-label maps (`Record<gameKey, string>`) + vocab.ts (engine-vocab re-export seam) — keyed by @workspace/game* types and read by both the domain view-builders and presentation (UNN-612 dissolved lib/ui here).
│   ├── pool.ts               The neutral `{ current, max }` display `Pool` + `hpPool`/`spPool` shapers (UNN-583/597) — owned by neither combat nor character, read by the rail, roster, watch, and dungeon token surfaces.
...
└── lib/                       The plumbing data tier (peer of domain): db/ auth/ storage/ realtime/ + the sync/write cluster. May import @workspace/game* + domain; may not import app/ or components/.
